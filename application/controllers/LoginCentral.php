...
class LoginCentral extends Zend_Controller_Action {
    ...
    public function logarAction() {
        ...
        // Mesmo que usuário e senha estejam corretos, ainda é verificado se o usuário está de fato ativo.
        // Anteriormente, esta verificação era feita junto com a autenticação mas isto impossibilitava uma distinção mais precisa do real motivo de recusa no login.
        ...
        if(!isset($objZendDbTableRowPessoa)) {
            $arrParams['erro'][] = 'Usuário inativo.';
            $this->forward('login', null, null, $arrParams);
            return;
        }

        $this->_inicializa($objZendDbTableRowUsuario, $objZendDbTableRowPessoa);
    ...
    private function _inicializa($objZendDbTableRowUsuario, $objZendDbTableRowPessoa) {
        ...
        $objZendDbTableRowGestor = $objZendDbTableRowPessoa->findEntity_Gestor()->current();
        if (isset($objZendDbTableRowGestor))
            $this->objZendSessionNamespace->objZendDbTableRowGestor = $objZendDbTableRowGestor;

        /*
        $objZendDbTableContribuinte = $objZendDbTableRowPessoa->findEntity_Contribuinte()->current();
		if(isset($objZendDbTableContribuinte))
			$this->_objZendSessionNamespace->objZendDbTableRowContribuinte = $objZendDbTableContribuinte;
        */
