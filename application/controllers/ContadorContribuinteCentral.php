...
class ContadorContribuinteCentral extends Zend_Controller_Action {
	...
	public function indexAction() {
		$this->getResponse()->setRedirect($this->view->url(array('controller' => 'contador-contribuinte', 'action' => 'listagem'), null, true))->sendResponse();
	}
	...
	public function alterarAction() {
		...
		$this->view->intTipoControle = self::TIPO_CONTROLE_DEFAULT;
		!Zend_Auth::getInstance()->hasIdentity() &&
			$this->view->intTipoControle = self::TIPO_CONTROLE_WORKFLOW;
	}
	...
	public function salvarAction() {
		...
		if($strAcao == 'inserir') { // É uma inserção.
			$objZendDbTableRowContribuinte->origem = self::ORIGEM_CRUD;
		...
		$objZendDbTableRowContribuinte->origem == self::ORIGEM_LOTE &&
			$objZendDbTableRowContribuinte->status == self::STATUS_PENDENTE &&
	...
	public function importarLoteAction() {
		...
		while($strLinha = fgets($rscFile)) {
			...
			try {
				...
				$objZendDbTableRowContribuinte->origem = self::ORIGEM_LOTE;
				$objZendDbTableRowContribuinte->status = self::STATUS_PENDENTE;
				$objZendDbTableRowContribuinte->cadastro_complementado = self::CADASTRO_COMPLEMENTADO_NAO;
				$objZendDbTableRowContribuinte->log_data = new Zend_Db_Expr("NOW()");
